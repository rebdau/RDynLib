...
```{r}
pth <- "data/database_QTOF_neg/CSV"
dir(pth)
```
...
base_path <- "data/database_QTOF_neg/Experimenten"
...
```{r}
dbGetQuery(con, "Select * from feature_matrix where expid = 12 LIMIT 10")
```	
...
```{r}
con <- dbConnect(RSQLite::SQLite(), "data/DyLib_QTOF_neg.sqlite")
dbListTables(con)
#dbDisconnect(con)
