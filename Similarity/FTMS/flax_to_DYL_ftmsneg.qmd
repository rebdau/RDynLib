...
```{r}
compound_add_path <- "D:/equip_msdata/database_FTMS_neg/CSV_add/compound_add.txt"
...
names(compound_add_df) <- sub("^GuN$", "GuN.C", names(compound_add_df))
names(compound_add_df) <- sub("^GaN$", "GaN.C", names(compound_add_df))
...
```{r}
con <- dbConnect(RSQLite::SQLite(), "data/DYL_FLAX_FTMSneg.sqlite")
dbListTables(con)
```


```{r}
gnps_add_path <- "D:/equip_msdata/database_FTMS_neg/CSV_add/gnps_add.txt"
