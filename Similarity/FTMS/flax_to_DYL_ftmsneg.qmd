...
This quarto document represents the full workflow to add ftmsneg flax to Dynlib
...
the query results show that the compound_id of the added experiment "266" start by 100912 which is correct
...
the query results show that the spectrum_id of the added experiment "266" start by 201035 which is correct
...
adding_Names_SQL <- function(con_merged, sim_table) {
  ...
  sim_msms <- sim_sub %>%
    inner_join(
      msms_spectrum %>% select(compound_id, dataOrigin),
      by = c("dataStorage" = "dataOrigin")
    )
  ...
  tryCatch({
    ...
    dbExecute(con_merged, "
      UPDATE ms_compound
      SET name = tmp_update_name.name
      FROM tmp_update_name
      WHERE ms_compound.compound_id = tmp_update_name.compound_id
    ")
