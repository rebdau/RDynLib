...
#add compound_add file to a sql table

```{r}
compound_add_path <- "D:/equip_msdata/database_QTOF_neg/CSV_add/compound_add.txt"

compound_add_df <- read.table(
  compound_add_path,
  header = TRUE,
  sep = "\t",
...
names(compound_add_df)[names(compound_add_df) == "COMPID"] <- "compound_id"

names(compound_add_df) <- make.unique(names(compound_add_df), sep = "_")

```

resolving duplicate names conflicts

```{r}
names(compound_add_df) <- sub("^CSPP_HuN$", "CSPP_HuN_C", names(compound_add_df))
names(compound_add_df) <- sub("^CSPP_HaN$", "CSPP_HaN_C", names(compound_add_df))
names(compound_add_df) <- sub("^CSPP_GuN$", "CSPP_GuN_C", names(compound_add_df))
names(compound_add_df) <- sub("^CSPP_GaN$", "CSPP_GaN_C", names(compound_add_df))
names(compound_add_df) <- sub("^CSPP_SuN$", "CSPP_SuN_C", names(compound_add_df))
names(compound_add_df) <- sub("^CSPP_SaN$", "CSPP_SaN_C", names(compound_add_df))
...
```{r}
cols_sql <- paste(
  sprintf('"%s" REAL', setdiff(names(compound_add_df), "compound_id")),
  collapse = ",\n  "
)

dbExecute(con_merged, "DROP TABLE IF EXISTS compound_add;")

create_sql <- sprintf("
CREATE TABLE compound_add (
  compound_id TEXT,
  %s
);
", cols_sql)

dbExecute(con_merged, create_sql)
...
```{r}
dbGetQuery(con_merged, "SELECT COUNT(*) FROM compound_add;")
dbGetQuery(con_merged, "SELECT * FROM compound_add LIMIT 5;")
```

#add gnps_add table

```{r}
con <- dbConnect(RSQLite::SQLite(), "data/DYL_FLAX_QTOFneg.sqlite")
dbListTables(con)
```


```{r}
gnps_add_path <- "D:/equip_msdata/database_QTOF_neg/CSV_add/gnps_add.txt"

gnps_add_df <- read.table(
  gnps_add_path,
  header = TRUE,
  sep = "\t",
  stringsAsFactors = FALSE,
...
names(gnps_add_df)[names(gnps_add_df) == "COMPID"] <- "compound_id"

names(gnps_add_df) <- make.unique(names(gnps_add_df), sep = "_")
...
```{r}
cols_sql <- paste(
  sprintf('"%s" REAL', setdiff(names(gnps_add_df), "compound_id")),
  collapse = ",\n  "
)

dbExecute(con, "DROP TABLE IF EXISTS gnps_add;")

create_sql <- sprintf("
CREATE TABLE gnps_add (
  compound_id TEXT,
  %s
);
", cols_sql)
...
dbGetQuery(con, "SELECT COUNT(*) FROM gnps_add;")
dbGetQuery(con, "SELECT * FROM gnps_add LIMIT 5;")
