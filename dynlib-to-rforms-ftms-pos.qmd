...
```{r}
dbf <- file.path("data", "DyLib_FTMS_pos2.sqlite")
...
base_path <- "data/database_FTMS_pos/Experimenten"
...
```{r}
con <- dbConnect(RSQLite::SQLite(), "data/DyLib_FTMS_pos2.sqlite")
dbListTables(con)
...
```{r}
tb <- dbGetQuery(con, "SELECT * FROM msms_spectrum WHERE acquisitionNum = 1928")
...
```{r}
spectraData(s, columns = c("msLevel", "compound_id", "nodename", "rtime", "retention_time", "spectrum_id"))
```
...
```{r}
#Function for adding spectra to a Compdb database
# spectra_object <- Spectra(msms_spectrum)
# cdb2 <- CompDb(con)
# insertSpectra(cdb2, spectra_object)
...
```{r}
con <- dbConnect(RSQLite::SQLite(), "data/DyLib_FTMS_pos.sqlite")
dbListTables(con)
```


```{r}
compound_add_path <- "D:/equip_msdata/database_FTMS_pos/CSV_add/compound_add.txt"
...
```{r}
names(compound_add_df) <- sub("^HuN$", "HuN_C", names(compound_add_df))
names(compound_add_df) <- sub("^HaN$", "HaN_C", names(compound_add_df))
names(compound_add_df) <- sub("^GuN$", "GuN_C", names(compound_add_df))
names(compound_add_df) <- sub("^GaN$", "GaN_C", names(compound_add_df))
names(compound_add_df) <- sub("^SuN$", "SuN_C", names(compound_add_df))
names(compound_add_df) <- sub("^SaN$", "SaN_C", names(compound_add_df))
...
dbExecute(con, "DROP TABLE IF EXISTS compound_add;")
...
dbGetQuery(con, "SELECT COUNT(*) FROM compound_add;")
dbGetQuery(con, "SELECT * FROM compound_add LIMIT 5;")
...
```{r}
gnps_add_path <- "D:/equip_msdata/database_FTMS_pos/CSV_add/gnps_add.txt"
