...
```{r}
pth <- "data/database_QTOF_pos/CSV"
dir(pth)
```
...
ms2_spectra_path <- "C:/Users/amentag/Desktop/these/Ahlam/these/equip_msdata/database_QTOF_pos/CSV/MS2spectra.csv"
...
dbf <- file.path("data", "DyLib_QTOF_pos.sqlite")
...
```{r}
library(CompoundDb)
md <- make_metadata(source = "DynLib QTOF pos", url = NA_character_,
...
base_path <- "data/database_QTOF_pos/Experimenten"
...
colnames(nodes_long)[colnames(nodes_long) == "node_name"] <- "nodename"
nodes_long
...
```{r}
dbGetQuery(con, "Select * from feature_matrix where expid = 6 LIMIT 10")
```	
...
```{r}
con <- dbConnect(RSQLite::SQLite(), "data/DyLib_QTOF_pos.sqlite")
dbListTables(con)
...
```{r}
spectraData(s, columns = c("msLevel", "compound_id", "nodename", "rtime", "retention_time"))
```
...
```{r}
con_merged <- dbConnect(SQLite(), "data/DyLib_QTOF_pos.sqlite")

compound_add_path <- "D:/equip_msdata/database_QTOF_pos/CSV_add/compound_add.txt"
...
```{r}
gnps_add_path <- "D:/equip_msdata/database_QTOF_pos/CSV_add/gnps_add.txt"
